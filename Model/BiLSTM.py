...
class BiLSTM(object):
    ...
    def _build_graph(self):
        ...
        with tf.name_scope('multi-bilstm-layer'):
            ...
            fw_cells = [DropoutWrapper(BasicLSTMCell(self._lstm_size, state_is_tuple=True), output_keep_prob=self._drop_out_placeholder) for _ in range(self._layers)]
            bw_cells = [DropoutWrapper(BasicLSTMCell(self._lstm_size, state_is_tuple=True), output_keep_prob=self._drop_out_placeholder) for _ in range(self._layers)]
            self._fw_cell = MultiRNNCell(fw_cells)
            self._bw_cell = MultiRNNCell(bw_cells)

            outputs, states = tf.nn.bidirectional_dynamic_rnn(self._fw_cell, self._bw_cell, embedded_x, dtype=tf.float64)
            output = tf.concat(outputs, axis=2)
            self._output = output

    def io_nodes(self):
        return self._x, self._vocab, self._drop_out_placeholder, self._output
